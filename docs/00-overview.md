...
구조체에 존재하지만 개별 필드는 mod_api에 노출돼 있지 않다 — 게임이 매치에서 이를 AI의 판단 품질로
변환한다(변환 로직 자체는 비공개). **모드는 그 변환 결과로 나온 최종 입력(`Input`)을 다룬다.**
...
| 서버/관리 로직 | `ModServerExtension` | 운영·DB·이벤트 (매치 시뮬 밖) |
