...
가장 큰 잠금 해제는 **인-매치 세계 상태**다. `GameCtx`는 조회 API(`get_entity`/`get_player`/위치/거리/
시야)를 갖지만 `think()`로 전달되는 공식 경로가 아직 없다([`01`](01-modapi-ai-surface.md) §GameCtx).
이 브리지가 검증되면 타깃·합류·포지셔닝 교정이 열린다 — 이 키트의 다음 목표.
